...
from smiles_helper import fetch_best_smiles, smiles_to_cas
...
if __name__ == "__main__":
    ...
    for chem in tqdm(test_chemicals, desc="SMILES Lookup"):
        ...
        if smiles:
            ...
            # Join with comma for display, avoid printing [''] as just empty string
            cas_string = ", ".join([cas for cas in cas_list if cas])
            if cas_string:
                print(f"  CAS(s): {cas_string}")
            else:
                print("  CAS(s): Not found")
        else:
            print("  CAS(s): Not found")
